# 📡 サイレーダー 2026-05-13 08:31 JST 試作版

このレポートは、2026-05-13 02:30 JST〜2026-05-13 08:30 JST に収集・観測した公開情報をもとに、サイバーセキュリティ関連トピックの温度感を試験的に整理したものです。
...
| 3 | [Hack: Local Privilege Escalation in Taskhost Windows Tasks (CVE-2025-60710).](#topic-12) | 74.0 | 82.0 | 81.0 | GitHub | 直近掲載済み・再掲抑制 |
...
| 6 | [Threats Making WAVs - Incident Response to a Cryptomining Attack](#topic-29) | 30.0 | 20.0 | 42.0 | GitHub | 直近掲載済み・再掲抑制 |
...
Palo Alto NetworksのPAN-OSに、認証不要で悪用されるおそれがある深刻な脆弱性CVE-2026-0300が公表されました。
対象はPA-SeriesおよびVM-Seriesで、User-ID Authentication Portalが有効な構成に影響するとされています。
ベンダーは限定的な実際の悪用を確認しており、修正版の提供前は回避策の適用が推奨されています。
ファイアウォール製品での認証不要のリモートコード実行につながる可能性があり、影響範囲によっては境界防御の前提を崩しかねません。
既に悪用が観測されている点から、一般的な脆弱性情報としてではなく、優先度の高い緊急対応案件として扱う必要があります。
...
- User-ID Authentication Portalの有効/無効と、外部から到達可能かどうかを確認する。
- 修正版が適用可能になるまで、アクセス元を信頼できる内部ゾーンに限定するなどの回避策を検討する。
- 影響を受けるPAN-OSのバージョンかどうかを棚卸しし、修正版の提供時期に合わせて迅速に更新計画を立てる。
...
Microsoftの2026年5月のPatch Tuesdayでは、120件超のCVEに対する修正が提供されました。
公開時点では、確認されている限りゼロデイとしての公的な悪用や公開済みの悪用情報は見当たらないとされていますが、Word関連を含む重要度の高い脆弱性は引き続き対応優先度が高いとみられます。
件数が多く、しかも複数の重要な脆弱性が含まれるため、通常運用の中でも見落としが起きやすい回です。
特に業務でWordやMicrosoft製品を広く使う環境では、早期適用の判断が影響範囲の抑制に直結します。
...
- Microsoft製品の月例更新を優先適用対象として整理し、特にWord関連の修正を早めに確認する。
- 社内のクライアント端末とサーバーで適用状況を分けて把握し、未適用端末の洗い出しを行う。
- 公開時点でゼロデイは確認されていなくても、今後の悪用情報の有無を継続監視する。
...
CVE-2025-60710は、Microsoft WindowsのTaskhost関連のタスク処理に起因するローカル権限昇格の脆弱性として報告されています。
CISAのKnown Exploited Vulnerabilitiesに掲載されており、公開PoCの言及もあるため、優先度の高い確認対象とみられています。
権限昇格の脆弱性は、侵入後により高い権限を得る足がかりになりやすく、被害の拡大につながる可能性があります。
さらにKEV掲載は、少なくとも実環境で悪用が確認されていることを示すため、運用上の緊急度が上がります。
...
- 対象Windows環境でCVE-2025-60710の影響有無を確認し、利用中の更新状況を点検する。
- 該当する端末やサーバーで不審な権限昇格の兆候、異常なタスク実行、管理者権限取得の痕跡を監視する。
- 優先度を上げてパッチ適用や緩和策の適用を進め、資産台帳上の該当範囲を早めに洗い出す。
...
Fortinet製品に複数の脆弱性が見つかっており、最も深刻なものはリモートコード実行につながる可能性があるとされています。
対象にはFortiAuthenticatorやFortiSandboxが含まれ、悪用された場合はシステム上で任意の操作を許してしまうおそれがあります。
認証や脅威検知に関わる製品が対象のため、影響範囲が広くなりやすい点が注目されています。公開情報では悪用観測の文脈もあり、早期の確認と対処が重要です。
...
- Fortinet製品の利用有無を確認し、該当製品があればベンダー案内に沿って修正状況を点検する。
- 管理者権限の運用状況を見直し、必要最小限の権限に抑える。
- 公開状況や監視アラートを確認し、通常と異なる操作や不審な変更がないか点検する。
...
### 6. Threats Making WAVs - Incident Response to a Cryptomining Attack
...
| [Microsoft製品向けの重要なパッチ公開、2026年5月12日](https://cisecurity.org/advisory/critical-patches-issued-for-microsoft-products-may-12-2026_2026-048) | 28.0 | 38.0 | 42.0 |
...
| [West Pharmaceuticalがランサムウェア攻撃を警告、業務に影響](https://therecord.media/west-pharmaceutical-warns-of-ransomware-attack-impacting-operations) | 28.0 | 30.0 | 42.0 |
...
| [米政府、Canvasへの大規模サイバー攻撃に関するInstructureの証言を要請](https://bleepingcomputer.com/news/security/us-govt-seeks-instructure-testimony-on-massive-canvas-cyberattack) | 28.0 | 20.0 | 42.0 |
...
| [OpenAI Sora 2の内部セキュリティ調査：マルチモーダルLLMを動かすシステムプロンプトの解明](https://mindgard.ai/resources/openai-sora-system-prompts) | 25.0 | 20.0 | 42.0 |
| [Must see: Geminiをハッキングする—多層的アプローチ](https://buganizer.cc/hacking-gemini-a-multi-layered-approach-md) | 25.0 | 20.0 | 42.0 |
| [主要経済国、AIの「構成要素リスト」の主要項目を明示](https://cyberscoop.com/g7-cisa-ai-sbom-security-guidance) | 25.0 | 20.0 | 42.0 |
